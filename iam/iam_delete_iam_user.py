        ...
        if response["AccessKeyMetadata"] != []:
            for data in response["AccessKeyMetadata"]:
                iam_client.delete_access_key(
                    UserName=user_name, AccessKeyId=data["AccessKeyId"]
                )
        ...
        if response["Certificates"] != []:
            for data in response["Certificates"]:
                iam_client.delete_signing_certificate(
                    UserName=user_name, CertificateId=data["CertificateId"]
                )
...
def delete_user_login_profile(iam_client, user_name):
    try:
        ...
        print(f"No User login Profile Found: {e}")
        ...
        if response["MFADevices"] != []:
            for data in response["MFADevices"]:
                iam_client.deactivate_mfa_device(
                    UserName=user_name, SerialNumber=data["SerialNumber"]
                )
        ...
        if response["AttachedPolicies"] != []:
            for data in response["AttachedPolicies"]:
                iam_client.detach_user_policy(
                    UserName=user_name, PolicyArn=data["PolicyArn"]
                )
...
def delete_permission_boundary(iam_client, user_name):
    try:
        ...
        print(f"No boundary permissions attached to user: {e}")
        ...
        if response["Groups"] != []:
            for data in response["Groups"]:
                iam_client.remove_user_from_group(
                    GroupName=data["GroupName"], UserName=user_name
                )
        ...
        if response["SSHPublicKeys"] != []:
            for data in response["SSHPublicKeys"]:
                iam_client.delete_ssh_public_key(
                    UserName=user_name, SSHPublicKeyId=data["SSHPublicKeyId"]
                )
...
def get_args():
    try:
        ...
        parser.add_argument(
            "--user_name",
            help="Enter IAM user name which you want to delete",
            type=str,
            required=True,
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"IAM User who will be deleted: {user_name}")
